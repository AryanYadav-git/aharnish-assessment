      ...
      <div className="grid grid-cols-3">
        ...
        <div className="flex flex-col gap-2 col-span-2">
          <p className="flex w-full text-lg justify-between pb-2 border-zinc-400 border-b">
            Total principal{" "}
            <span className="text-green-800 font-semibold">{`$ ${amount}`}</span>
          </p>
          <p className="flex w-full text-lg justify-between pb-2 border-zinc-400 border-b">
            Total interest payments{" "}
            <span className="text-green-800 font-semibold">{`$ ${totalInterestPayments}`}</span>
          </p>
          <p className="flex w-full text-lg justify-between pb-2 border-zinc-400 border-b">
            Total loan payments{" "}
            <span className="text-green-800 font-semibold">{`$ ${totalLoanPayments}`}</span>
          </p>
          <p className="flex w-full text-lg justify-between pb-2 border-zinc-400 border-b">
            Payoff date{" "}
            <span className="text-green-800 font-semibold">{`${payoffDate.month}/${payoffDate.year}`}</span>
          </p>
        </div>
      </div>
    </div>
